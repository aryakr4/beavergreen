...
export default function ReviewForm({ locationId, onSubmitted }: ReviewFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 rounded-lg border border-fog/30 bg-white p-4">
      <label className="flex flex-col text-sm text-basalt/80">
        ...
        <select
          ...
          className="mt-1 w-24 rounded border border-fog/50 px-2 py-1"
        >
          {[5, 4, 3, 2, 1].map((n) => (
      ...
      <button
        type="submit"
        disabled={submitting}
        className="self-start rounded-full bg-glacial px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
      >
        Submit review
      </button>
    </form>
